...
export class StateTracker {
	...
	private ruleStates = new Map<
		string,
		{ triggered: boolean; firedCount: number }
	>();
	...
	update(toolName: string, resultChars: number, isError: boolean): void {
		...
		this.toolChars.set(
			toolName,
			(this.toolChars.get(toolName) ?? 0) + resultChars,
		);
	...
	matches(condition: StateCondition): boolean {
		...
		if (kind === "errors") {
			...
			value = (condition.tools ?? []).reduce(
				(sum, t) => sum + (this.toolChars.get(t) ?? 0),
				0,
			);
		} else {
			value = (condition.tools ?? []).reduce(
				(sum, t) => sum + (this.toolCounts.get(t) ?? 0),
				0,
			);
		}
	...
	markTriggered(ruleKey: string): number {
		const s = this.ruleStates.get(ruleKey) ?? {
			triggered: true,
			firedCount: 0,
		};
		s.triggered = true;
