...
describe("roadmap JSON block 规则 — bash 边缘情况", () => {
	it("awk 处理 roadmap.json → block", () => {
		expect(isBlocked("bash", { command: "awk '{print $1}' roadmap.json" })).toBe(true);
	});
	...
	it("cat 后跟非 roadmap 的 roadmap.json → block", () => {
		expect(isBlocked("bash", { command: "cat -n /home/lain/.pi/roadmap/roadmap.json" })).toBe(true);
	});

	it("bash echo 不碰 roadmap → 不拦截", () => {
		expect(isBlocked("bash", { command: "echo 'roadmap.json is a config file'" })).toBe(false);
	});

	it("bash grep roadmap.json 但不读写文件 → 不拦截", () => {
		expect(isBlocked("bash", { command: "grep 'status' .pi/roadmap/roadmap.json" })).toBe(false);
	});
	...
	it("bash git 操作涉及 roadmap → 不拦截", () => {
		expect(isBlocked("bash", { command: "git add .pi/roadmap/roadmap.json" })).toBe(false);
	});

	it("bash diff 比较 roadmap → 不拦截", () => {
		expect(isBlocked("bash", { command: "diff .pi/roadmap/roadmap.json .pi/roadmap/roadmap.json.bak" })).toBe(false);
	});
});
