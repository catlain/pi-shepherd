...
describe("checkWorktrees", () => {
	beforeAll(() => {
		...
		try {
			mkdirSync(TEST_WT, { recursive: true });
		} catch {}
	});
	afterAll(() => {
		try {
			rmSync(join(process.cwd(), ".worktrees"), {
				recursive: true,
				force: true,
			});
		} catch {}
	...
	it("已合并 worktree 分支自动清理并通知", () => {
		...
		expect(n).toHaveBeenCalledWith(
			expect.stringContaining("自动清理了 1 个已合并 worktree"),
			"info",
		);
	});
	...
	it("未合并分支触发 warning 通知", () => {
		mockedExecFileSync.mockImplementation((_c: string, a: string[]) => {
			if (a[0] === "branch" && a.includes("--list"))
				return "  worktree/feature-x\n";
			if (a[0] === "log" && a.includes("main..worktree/feature-x"))
				return "abc123\n";
		...
		checkWorktrees({ notify: n });
		expect(n).toHaveBeenCalledWith(
			expect.stringContaining("未合并 worktree"),
			"warning",
		);
	});
	...
	it("未合并分支无未提交改动时只显示 ahead 数", () => {
		...
		checkWorktrees({ notify: n });
		expect(n).toHaveBeenCalledWith(
			expect.stringContaining("2 个未合并提交"),
			"warning",
		);
	});
	...
	it("多条未合并分支显示完整列表", () => {
		...
		checkWorktrees({ notify: n });
		expect(n).toHaveBeenCalledWith(
			expect.stringContaining("2 个未合并 worktree"),
			"warning",
		);
		expect(n).toHaveBeenCalledWith(
			expect.stringContaining("/worktree-merge"),
			"warning",
		);
	});
