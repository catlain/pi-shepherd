...
describe("StateTracker", () => {
	...
	describe("matches", () => {
		it("calls gte 条件：达到阈值", () => {
			...
			tracker.update("tool_a", 0, false);
			expect(
				tracker.matches({ tools: ["tool_a"], gte: 2, countKind: "calls" }),
			).toBe(true);
		});
		...
		it("calls lte 条件：未超过上限", () => {
			tracker.update("tool_a", 0, false);
			expect(
				tracker.matches({ tools: ["tool_a"], lte: 5, countKind: "calls" }),
			).toBe(true);
		});
		...
		it("calls lte 条件：超过上限", () => {
			...
			tracker.update("tool_a", 0, false);
			expect(
				tracker.matches({ tools: ["tool_a"], lte: 2, countKind: "calls" }),
			).toBe(false);
		});
		...
		it("chars gte 条件：达到阈值", () => {
			...
			expect(
				tracker.matches({ tools: ["tool_a"], gte: 10000, countKind: "chars" }),
			).toBe(true);
		});
		...
		it("chars gte 条件：未达到阈值", () => {
			...
			expect(
				tracker.matches({ tools: ["tool_a"], gte: 1000, countKind: "chars" }),
			).toBe(false);
		});
		...
		it("多个工具的 chars 求和判断", () => {
			...
			expect(
				tracker.matches({ tools: ["a", "b"], gte: 1000, countKind: "chars" }),
			).toBe(true);
		});
	});
